...
const ClinicalBrief: React.FC = () => {
  ...
  if (!parsedInfo) {
    return <Panel title="Clinical Brief"><div className="text-center text-sm text-(--color-text-muted) p-4">Awaiting analysis...</div></Panel>;
  }
    ...
    <Panel title="Interactive Clinical Brief" className="h-full">
      <div className="divide-y divide-(--color-border) -mt-4">
        <Section title="Study Type">
          <EditableField value={parsedInfo.studyType?.value} onSave={handleSimpleUpdate('studyType')} className="text-sm text-(--color-text-bright) font-semibold" />
        </Section>
        ...
        <Section title="Reason for Study">
          <EditableField value={parsedInfo.reasonForStudy?.value} onSave={handleSimpleUpdate('reasonForStudy')} className="text-sm text-(--color-text-default)" />
        </Section>
        <Section title="Clinical History">
          <EditableField value={parsedInfo.clinicalHistory?.value} onSave={handleSimpleUpdate('clinicalHistory')} className="text-sm text-(--color-text-default) whitespace-pre-wrap" />
        </Section>
          ...
          <Section title="Lab Results">
            <div className="space-y-2 text-sm">
              {parsedInfo.labs.map((lab, index) => (
                <div key={index} className="flex justify-between items-center text-(--color-text-default)">
                  <EditableField value={lab.name} onSave={handleLabUpdate(index, 'name')} className="font-semibold text-(--color-text-bright)" />
