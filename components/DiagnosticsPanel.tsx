        ...
        <div className="fixed bottom-4 right-4 bg-black/80 backdrop-blur-md border border-(--color-border) rounded-lg shadow-2xl z-50 w-full max-w-lg h-96 flex flex-col animate-slide-in-bottom">
            <header className="flex items-center justify-between p-3 border-b border-(--color-secondary)/30 shrink-0">
                ...
                <div className="flex items-center space-x-2">
                    <button onClick={clearDiagnostics} title="Clear Log" className="p-1 text-(--color-text-muted) hover:text-white transition-colors">
                        <TrashIcon className="h-4 w-4" />
                    </button>
                    <button onClick={toggleDiagnosticsPanel} title="Close" className="p-1 text-(--color-text-muted) hover:text-white transition-colors">
            ...
            <div className="p-3 overflow-y-auto grow font-mono text-xs">
                {messages.length === 0 ? (
                    ...
                    messages.map((msg, index) => (
                        <div key={index} className={`py-1 border-b border-(--color-border)/50 ${msg.type === 'error' ? 'text-(--color-danger-text)' : 'text-(--color-text-default)'}`}>
                            <span className="text-(--color-primary) opacity-80 mr-2">{msg.timestamp.toLocaleTimeString()}</span>
                            <span>{msg.message}</span>
                            {msg.data && <pre className="text-(--color-text-muted) text-xs whitespace-pre-wrap pl-4 mt-1 opacity-70">{msg.data}</pre>}
                        </div>
                    ))
                )}
