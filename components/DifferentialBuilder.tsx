...
const LikelihoodBadge: React.FC<{ likelihood: 'High' | 'Medium' | 'Low' }> = ({ likelihood }) => {
    ...
    return (
        <span className={`px-2 py-0.5 text-xs font-semibold rounded-full border ${colorClasses[likelihood]}`}>
            {likelihood}
        </span>
    );
};
        ...
        <div className="flex flex-col h-full">
            <div className="grow overflow-y-auto pr-2">
             ...
             <div className="flex justify-end pt-4 mt-auto">
                ...
                >
                    {isSynthesizing ? 'Synthesizing...' : `Synthesize Impression (${selectedDifferentials.length})`}
                </ActionButton>
            </div>
        </div>
    );
