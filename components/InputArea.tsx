  ...
  return (
    <div className="relative panel-base animate-fade-in p-0">
      <textarea
        ...
        className="glass-input w-full bg-transparent border-none h-64 p-4 pr-12 pb-16 rounded-lg focus:outline-none resize-none transition-shadow duration-200 text-[var(--color-text-default)] disabled:opacity-50"
        disabled={isLoading || !isAiReady || isDisabled}
      />
       {isListening && interimTranscript && (
        <span className="absolute top-4 left-4 text-[var(--color-text-muted)] opacity-50 pointer-events-none whitespace-pre-wrap">{liveValue}</span>
      )}
      <div className="absolute right-4 bottom-4 flex items-center space-x-2">
        {isSpeechRecognitionSupported && micPermission === 'granted' && (
          <button
            ...
            className={`p-3 rounded-full transition-all duration-300 ${isListening ? 'bg-red-500/20 animate-pulse text-red-400' : 'text-[var(--color-text-muted)] hover:bg-[var(--color-interactive-bg-hover)] hover:text-white'}`}
          >
