...
export const parseMarkdownToHTML = (content: string): string => {
  ...
  htmlContent = htmlContent.replace(
    /@@(.*?)@@/g,
    '<span class="text-indigo-400 font-semibold tracking-wide">$1</span>'
  );
  ...
  htmlContent = htmlContent.replace(
    /^([A-Za-z\s’'-]+):/gm,
    '<strong class="block text-slate-200 mt-4 mb-1 uppercase text-xs tracking-wider">$1:</strong>'
  );
  ...
  htmlContent = htmlContent
    .replace(
      /^\s*###\s*(.*$)/gim,
      '<h4 class="font-semibold text-[var(--color-primary)] text-sm mb-2 mt-3 first:mt-0">$1</h4>'
    )
    .replace(
      /^\s*\*\s*(.*$)/gim,
      "<li class=\"relative before:content-['•'] before:absolute before:left-[-1em] before:text-[var(--color-primary)]\">$1</li>"
    );

  // 4. Process inline elements like bold text
  htmlContent = htmlContent.replace(
    /\*\*(.*?)\*\*/g,
    '<strong class="text-[var(--color-text-bright)]">$1</strong>'
  );

  // 5. Group consecutive list items into <ul> tags
  const groupedHtml = htmlContent.replace(/<\/li>\s*<li/g, "</li><li");
  const finalHtml = groupedHtml.replace(
    /(<li.*<\/li>)+/g,
    (match) => `<ul class="space-y-1.5 pl-4 my-2">${match}</ul>`
  );
