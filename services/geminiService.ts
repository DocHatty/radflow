...
export const generateImpressionistBackground = async (apiKey?: string, availableModels?: string[]): Promise<string> => {
    ...
    if (apiKey) {
        ...
        if (availableModels && availableModels.length > 0) {
            ...
            if (imagenModels.length > 0) {
                ...
                console.log("Smart Selection: Using detected Imagen models:", modelsToTry);
            }
        }
        ...
        if (modelsToTry.length === 0) {
            ...
            console.log("Smart Selection: Using default fallback models:", modelsToTry);
        }

        for (const model of modelsToTry) {
            try {
                console.log(`Attempting background generation with model: ${model}`);
                ...
                if (base64Image) {
                    console.log(`Success: Background generated by ${model}`);
    ...
    if (apiKey) {
        console.warn(`All AI background generation attempts failed. Last error: ${lastError?.message || JSON.stringify(lastError)}`);
    }
    console.log("Switching to local fallback image.");

    const randomFallback = LOCAL_FALLBACK_IMAGES[Math.floor(Math.random() * LOCAL_FALLBACK_IMAGES.length)];
